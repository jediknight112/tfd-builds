...
export class ExternalComponentSelector {
  openExternalComponentSelector(equipmentType) {
    ...
    document.querySelectorAll('.external-component-filter-btn[data-tier]').forEach(btn => {
      btn.classList.remove('active');
    });
    document.querySelector('.external-component-filter-btn[data-tier="all"]')?.classList.add('active');
  ...
  renderExternalComponentSelectorGrid(searchQuery = '', tierFilter = 'all') {
    ...
    let filteredComponents = state.externalComponents.filter(component => {
      ...
      if (searchQuery) {
        ...
        const nameMatch = component.external_component_name.toLowerCase().includes(searchLower);
    ...
    filteredComponents.sort((a, b) => {
      ...
      const tierDiff = (tierOrder[a.external_component_tier_id] || 0) - (tierOrder[b.external_component_tier_id] || 0);
    ...
    if (filteredComponents.length === 0) {
      grid.innerHTML = '<div class="col-span-full text-center py-8 text-gray-400">No components found</div>';
      return;
    }
  ...
  createExternalComponentCard(component) {
    ...
    // Get level 100 stats
    const level100Stat = component.base_stat?.find(stat => stat.level === 100);
      ...
      <div class="flex flex-col h-full">
        ...
        ${level100Stat ? `
          <div class="space-y-2 text-sm border-t border-steel-grey/20 pt-3">
            ...
            ${component.set_option_detail && component.set_option_detail.length > 0 ? `
    ...
    card.addEventListener('click', () => this.selectExternalComponent(component));
  ...
  renderExternalComponentsDisplay() {
    ...
    equipmentTypes.forEach(equipmentType => {
      ...
      if (!component) {
        ...
        // Component selected
        const level100Stat = component.base_stat?.find(stat => stat.level === 100);
        const tierClass = component.external_component_tier_id ? `tier-${component.external_component_tier_id.replace('Tier', '')}` : '';
  ...
  setupEventListeners() {
    ...
    if (searchInput) {
      searchInput.addEventListener('input', (e) => {
        ...
        const tierFilter = document.querySelector('.external-component-filter-btn[data-tier].active')?.dataset.tier || 'all';
    ...
    document.querySelectorAll('.external-component-filter-btn[data-tier]').forEach(btn => {
      btn.addEventListener('click', (e) => {
        // Update active state
        document.querySelectorAll('.external-component-filter-btn[data-tier]').forEach(b => b.classList.remove('active'));
    ...
    // Close button
    const closeBtn = document.getElementById('close-external-component-selector');
    if (closeBtn) {
      closeBtn.addEventListener('click', () => this.closeExternalComponentSelector());
    }
