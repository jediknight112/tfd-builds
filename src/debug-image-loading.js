...
window.debugImageLoading = function () {
  ...
  console.log(
    '- Worker API Key:',
    state.apiKeys.workerApiKey ? '✅ Set' : '❌ Missing'
  );
  console.log(
    '- Nexon API Key:',
    state.apiKeys.nexonApiKey ? '✅ Set' : '❌ Missing'
  );

  if (state.apiKeys.workerApiKey) {
    console.log(
      '- Worker Key (first 10 chars):',
      state.apiKeys.workerApiKey.substring(0, 10) + '...'
    );
  }
  if (state.apiKeys.nexonApiKey) {
    console.log(
      '- Nexon Key (first 10 chars):',
      state.apiKeys.nexonApiKey.substring(0, 10) + '...'
    );
  }
  ...
  if (cacheImages.length > 0) {
    ...
    cacheImages.forEach((img, i) => {
      ...
      console.log(
        '- Has loading class:',
        img.classList.contains('loading-image')
      );
      console.log(
        '- Has loaded class:',
        img.classList.contains('loaded-image')
      );
    });
  }
